  ...
  if todownfile.media is None:
    try:
      ...
      if os.path.isdir(direct_link_path):
        ...
        link = public_link
        await megaupmsg.edit(f"**Successfully Uploaded To Mega.nz** \n\n**Link:** `{link}` \n\n**Powered by @ReGALTOs_BotZ**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Mega.nz Link 📥", url=f"{link}")]]))
        os.remove(toupload)
      return
    except Exception as e:
...
async def importurlf(client: Client, message: Message):
  ...
  try:
    ...
    try:
      ...
      await message.reply_text(f"**Successfully Imported 😌** \n\n**Link:** `{imported_link}` \n\n**Powered by @ReGALTOs_BotZ**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Imported Link 📥", url=f"{imported_link}")]]))
    except Exception as e:
